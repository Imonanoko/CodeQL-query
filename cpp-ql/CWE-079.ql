// Auto-generated; CWE-079; number of APIs 179
import cpp
...
predicate isTargetApi(Function target, string qn) {
  ...
  target.getQualifiedName().matches("std%::ostringstream%") and qn = "std::ostringstream" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "std") and
    memberFunc.getName() = "ostringstream" and
    qn = "std::ostringstream"
  ) or
  target.getQualifiedName().matches("std%::stringstream%") and qn = "std::stringstream" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "std") and
    memberFunc.getName() = "stringstream" and
    qn = "std::stringstream"
  ) or
  ...
  target.getQualifiedName().matches("std%::wostringstream%::operator<<%") and qn = "std::wostringstream::operator<<" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("std", "wostringstream") and
    memberFunc.getName() = "operator<<" and
    qn = "std::wostringstream::operator<<"
  ) or
  target.getQualifiedName().matches("std%::wstringstream%::operator<<%") and qn = "std::wstringstream::operator<<" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("std", "wstringstream") and
    memberFunc.getName() = "operator<<" and
    qn = "std::wstringstream::operator<<"
  ) or
  ...
  target.getQualifiedName().matches("fprintf_s%") and qn = "fprintf_s" or
  target.getQualifiedName().matches("sprintf_s%") and qn = "sprintf_s" or
  target.getQualifiedName().matches("snprintf_s%") and qn = "snprintf_s" or
  target.getQualifiedName().matches("std%::format%") and qn = "std::format" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "std") and
  ...
  target.getQualifiedName().matches("boost%::format%::str%") and qn = "boost::format::str" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("boost", "format") and
    memberFunc.getName() = "str" and
    qn = "boost::format::str"
  ) or
  target.getQualifiedName().matches("boost%::format%::operator%%") and qn = "boost::format::operator%" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("boost", "format") and
    memberFunc.getName() = "operator%" and
    qn = "boost::format::operator%"
  ) or
  target.getQualifiedName().matches("Poco%::Format%") and qn = "Poco::Format" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
  ...
  target.getQualifiedName().matches("glog%::LOG_EVERY_N%") and qn = "glog::LOG_EVERY_N" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "glog") and
    memberFunc.getName() = "LOG_EVERY_N" and
    qn = "glog::LOG_EVERY_N"
  ) or
  target.getQualifiedName().matches("glog%::LOG_FIRST_N%") and qn = "glog::LOG_FIRST_N" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "glog") and
    memberFunc.getName() = "LOG_FIRST_N" and
    qn = "glog::LOG_FIRST_N"
  ) or
  ...
  target.getQualifiedName().matches("htmlspecialchars%") and qn = "htmlspecialchars" or
  target.getQualifiedName().matches("htmlentities%") and qn = "htmlentities" or
  target.getQualifiedName().matches("escape_html%") and qn = "escape_html" or
  target.getQualifiedName().matches("sanitize_html%") and qn = "sanitize_html" or
  target.getQualifiedName().matches("render_html%") and qn = "render_html" or
  target.getQualifiedName().matches("render_template%") and qn = "render_template" or
  target.getQualifiedName().matches("render_to_string%") and qn = "render_to_string" or
  target.getQualifiedName().matches("crow%::response%::set_body%") and qn = "crow::response::set_body" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("crow", "response") and
    memberFunc.getName() = "set_body" and
    qn = "crow::response::set_body"
  ) or
  ...
  target.getQualifiedName().matches("QTextStream%") and qn = "QTextStream" or
  target.getQualifiedName().matches("QTextStream%::operator<<%") and qn = "QTextStream::operator<<" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QTextStream") and
    memberFunc.getName() = "operator<<" and
    qn = "QTextStream::operator<<"
  ) or
  target.getQualifiedName().matches("QTextStream%::write%") and qn = "QTextStream::write" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QTextStream") and
    memberFunc.getName() = "write" and
    qn = "QTextStream::write"
  ) or
  target.getQualifiedName().matches("QTextStream%::flush%") and qn = "QTextStream::flush" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QTextStream") and
    memberFunc.getName() = "flush" and
    qn = "QTextStream::flush"
  ) or
  target.getQualifiedName().matches("QTextBrowser%::setHtml%") and qn = "QTextBrowser::setHtml" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QTextBrowser") and
    memberFunc.getName() = "setHtml" and
    qn = "QTextBrowser::setHtml"
  ) or
  target.getQualifiedName().matches("QTextBrowser%::append%") and qn = "QTextBrowser::append" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QTextBrowser") and
    memberFunc.getName() = "append" and
    qn = "QTextBrowser::append"
  ) or
  target.getQualifiedName().matches("QLabel%::setText%") and qn = "QLabel::setText" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QLabel") and
    memberFunc.getName() = "setText" and
    qn = "QLabel::setText"
  ) or
  target.getQualifiedName().matches("QLabel%::setTextFormat%") and qn = "QLabel::setTextFormat" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QLabel") and
    memberFunc.getName() = "setTextFormat" and
    qn = "QLabel::setTextFormat"
  ) or
  target.getQualifiedName().matches("QLabel%::setTextInteractionFlags%") and qn = "QLabel::setTextInteractionFlags" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "QLabel") and
    memberFunc.getName() = "setTextInteractionFlags" and
    qn = "QLabel::setTextInteractionFlags"
  ) or
  ...
  target.getQualifiedName().matches("Mongoose%::mg_vprintf%") and qn = "Mongoose::mg_vprintf" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "Mongoose") and
    memberFunc.getName() = "mg_vprintf" and
    qn = "Mongoose::mg_vprintf"
  ) or
  ...
  target.getQualifiedName().matches("CivetServer%::mg_send_file%") and qn = "CivetServer::mg_send_file" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "CivetServer") and
    memberFunc.getName() = "mg_send_file" and
    qn = "CivetServer::mg_send_file"
  ) or
  ...
  target.getQualifiedName().matches("cpprestsdk%::http_response%::set_body_stream%") and qn = "cpprestsdk::http_response::set_body_stream" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("cpprestsdk", "http_response") and
    memberFunc.getName() = "set_body_stream" and
    qn = "cpprestsdk::http_response::set_body_stream"
  ) or
  ...
  target.getQualifiedName().matches("drogon%::HttpResponse%::setExpiredTime%") and qn = "drogon::HttpResponse::setExpiredTime" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("drogon", "HttpResponse") and
    memberFunc.getName() = "setExpiredTime" and
    qn = "drogon::HttpResponse::setExpiredTime"
  ) or
  target.getQualifiedName().matches("drogon%::HttpResponse%::setCustomStatusCode%") and qn = "drogon::HttpResponse::setCustomStatusCode" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("drogon", "HttpResponse") and
    memberFunc.getName() = "setCustomStatusCode" and
    qn = "drogon::HttpResponse::setCustomStatusCode"
  ) or
  ...
  target.getQualifiedName().matches("cppcms%::http%::context%::out%") and qn = "cppcms::http::context::out" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("cppcms::http", "context") and
    memberFunc.getName() = "out" and
    qn = "cppcms::http::context::out"
  ) or
  ...
  target.getQualifiedName().matches("fcgi%::FCGX_PutChar%") and qn = "fcgi::FCGX_PutChar" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
    memberFunc.getDeclaringType().hasQualifiedName("", "fcgi") and
    memberFunc.getName() = "FCGX_PutChar" and
    qn = "fcgi::FCGX_PutChar"
  ) or
  ...
  target.getQualifiedName().matches("crow%::response%") and qn = "crow::response" or
  exists(MemberFunction memberFunc | 
    memberFunc = target and
